...
namespace DowntownRP.Game.Commands
{
    public class Admin : Script
    {
        ...
        [Command("ponerropa")]
        public async Task CMD_ponerropa(Player player, int id, int slot, int drawable, int texture)
        {
            if (!player.HasData("USER_CLASS")) return;
            Data.Entities.User user = player.GetData<Data.Entities.User>("USER_CLASS");
            //var user = player.GetExternalData<Data.Entities.User>(0);
        ...
        public static void CMD_infoBanco(Player player, int id)
        {
            Player target = Utilities.PlayerId.FindPlayerById(id);
            if (!player.HasData("USER_CLASS")) return;
            Data.Entities.User user = player.GetData<Data.Entities.User>("USER_CLASS");
            if (user.adminLv >= 3)
            {
                Data.Entities.User targetu = target.GetData<Data.Entities.User>("USER_CLASS");
        ...
        public static void CMD_ResetPin(Player player, int id, int newpin)
        {
            Player target = Utilities.PlayerId.FindPlayerById(id);
            if (!player.HasData("USER_CLASS")) return;
            Data.Entities.User user = player.GetData<Data.Entities.User>("USER_CLASS");
